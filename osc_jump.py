...
class OscGains:
    ...
    kd_com: np.ndarray
    w_com: np.ndarray
    kp_left_foot: np.ndarray
    kd_left_foot: np.ndarray
    w_left_foot: np.ndarray
    kp_right_foot: np.ndarray
    kd_right_foot: np.ndarray
    w_right_foot: np.ndarray
    kp_base: np.ndarray
    kd_base: np.ndarray
    w_base: np.ndarray
    w_vdot: float
...
class OperationalSpaceWalkingController(LeafSystem):
    ...
    def SetupAndSolveQP(self, context: Context) -> Tuple[np.ndarray, MathematicalProgram]:
        ...
        lb = np.array([-999, -999], dtype=np.float64)
        ub = np.zeros(2).reshape(-1, 1)
        prog.AddLinearConstraint(A_ineq, lb, ub, lambda_c)
